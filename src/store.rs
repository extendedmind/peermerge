...
where
    T: RandomAccess<Error = Box<dyn std::error::Error + Send + Sync>> + Debug + Send,
{
    async fn add_discovery_key_to_state(&mut self, discovery_key: &[u8; 32]) {
        self.doc_state
            .peer_discovery_keys
            .push(discovery_key.clone());
...
impl HypercoreStore<RandomAccessMemory> {
    pub async fn new_memory(
        ...
        doc_state.peer_discovery_keys.push(discovery_key);
...
impl DocStore<RandomAccessMemory> {
    ...
    pub async fn add_doc_memory(&mut self, mut doc: Automerge) -> String {
        ...
        // Write the state
        self.add_discovery_key_to_state(&discovery_key).await;

        // Insert a hypercore store to the doc store
        let hypercore_store =
            HypercoreStore::new_memory(discovery_key.clone(), hypercore, Some(doc)).await;
    ...
    pub async fn register_doc_memory(&mut self, doc_url: &str) {
        ...
        // Insert a hypercore store to the doc store
        let hypercore_store =
            HypercoreStore::new_memory(discovery_key.clone(), hypercore, None).await;
        self.docs.insert(discovery_key, hypercore_store);
    }
}
